...
const ResourceCard: React.FC<ResourceCardProps> = ({ resource }) => {
    ...
    return (
        <a 
            ...
            className="group bg-white rounded-lg shadow-md hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#0053b4] transition-all duration-300 p-5 flex flex-col h-full"
        >
            <div className='flex items-start space-x-4'>
                <div className="flex-shrink-0 w-12 h-12 bg-[#aae4fa]/50 rounded-lg flex items-center justify-center">
                    <DocumentIcon />
                </div>
                <div className="flex-1">
                     <h3 className="text-lg font-bold text-[#051632] group-hover:text-[#0053b4] transition-colors break-words">
                        {title}
                    </h3>
                </div>
            </div>
...
export const ResourceList: React.FC<ResourceListProps> = ({ resources }) => {
    if (resources.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-full text-center bg-white rounded-lg shadow-lg p-12">
